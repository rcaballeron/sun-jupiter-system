...
\chapter{Metodología}\label{ch:octavo-capitulo}
\section{Catálogos de datos}
\subsection{Gaia}
El catálogo Gaia \citep{Mignard2005} es un mapa tridimensional completo de la Vía Láctea, creado utilizando datos del telescopio espacial Gaia. Éste contiene datos de alrededor de 1460 millones de estrellas, entre los que se encuentran sus posiciones en el cielo, paralajes y movimientos propios. Además, incluye sus magnitudes en diferentes bandas espectrales para una gran cantidad de ellas. El catálogo Gaia es particularmente útil para estudiar estrellas similares al Sol, ya que proporciona un censo único de estrellas en un entorno de 100 pc alrededor de éste. Esto lo convierte en un recurso invaluable a la hora de comprender mejor las propiedades y la evolución de este tipo de estrellas.\par

\subsection{Gaia-ESO Spectroscopic Survey - (GES)}
El estudio Gaia-ESO (Gaia-ESO Survey, GES) \citep{Gilmore2012,Randich2013,Randich2022} es un catálogo público enfocado al análisis espectroscópico de un número importante de estrellas recogidas en el catálogo Gaia. Se llevó a cabo con el instrumento espectrógrafo multielemento de fibra de gran conjunto (Fibre Large Array Multi Element Spectrograph, FLAMES) y el espectrógrafo Echelle ultravioleta y visual (Ultraviolet and Visual Echelle Spectrograph, UVES) instalados en el telescopio muy grande (Very Large Telescope, VLT).\par

GES ha producido registros espectroscópicos de muy alta calidad de alrededor de 110 000 estrellas de la Vía Láctea, en el campo y en cúmulos abiertos (Open Cluster, OC), hasta la magnitud 19. Ha proporcionado velocidades radiales y parámetros estelares de buena calidad para una gran parte de sus estrellas objetivo, así como las abundancias elementales de hasta 31 elementos para aquellos objetivos observados con FLAMES y UVES. Adicionalmente, para aproximadamente un tercio de la muestra ha conseguido medir las abundancias de litio. Esto hace del catálogo GES un recurso único en lo que se refiere a la observación de estrellas de todos los tipos espectrales con análisis dedicados y especializados, lo que hace de este catálogo una herramienta particularmente útil para estudiar estrellas similares al Sol.\par

\section{Herramientas}
\subsection{Modules for Experiments in Stellar Astrophysics - MESA}
Para la realización de esta tesis nos hemos apoyado en la herramienta de evolución estelar MESA (Modules for Experiments in Stellar Astrophysics). MESA ha jugado un papel fundamental en nuestra investigación y por ello, debido a su importancia, le hemos dedicado un capítulo en exclusiva en el que entramos a fondo en sus características, estructura, módulos que lo componen, y muy especialmente en cómo extender la física que incorporan sus modelos mediante la programación de nuestras propias rutinas.\par
...
Antes de poder realizar la consulta, hay que localizar el catálogo sobre el que ejecutar la búsqueda. Mediante el protocolo de acceso a tablas (Table Access Protocol, TAP) TOPCAT ofrece la posibilidad de realizar búsquedas entre los catálogos que soportan TAP mediante el uso de palabras claves o propiedades de los catálogos. En la figura \ref{fig:tap_ges_service} vemos el resultado ofrecido tras realizar una búsqueda en los catálogos disponibles que están relacionados con GES.\par
...
De manera similar, utilizamos TOPCAT para localizar catálogos con información de cúmulos abiertos (figura \ref{fig:tap_query_oc}). Una vez seleccionado el catálogo que nos interesa, TOPCAT nos ofrece la posibilidad de obtener detalles adicionales sobre los esquemas y tablas que los conforman. Esta información es muy útil a la hora de realizar nuestras consultas, ya que de ella podemos extraer no solo las columnas incluidas en las diferentes tablas, sino también el tipo de información que contiene (algo que se hace necesario cuando el nombre de la columna no es suficientemente explicativo), tipo de datos (numérico, texto, fecha...), columnas clave, referencia entre tablas (claves foráneas) y más información relevante. Este conjunto de información adicional es lo que se conoce como meta-información o metadatos asociados a la tabla (ver figura \ref{fig:tap_table_metadata}).\par
...
La combinación de los datos de los catálogos Gaia y GES nos puede proporcionar una visión completa de las propiedades y evolución de las estrellas similares al Sol. El catálogo Gaia proporciona datos de posición precisos y paralajes, mientras que la GES ofrece información espectroscópica detallada. Esta combinación nos permite una comprensión más completa de estas estrellas, incluyendo su cinemática, composiciones químicas y potencial para albergar planetas. Al proporcionar una perspectiva más amplia y detallada de estas estrellas, podemos obtener una comprensión más profunda de las propiedades y evolución de las estrellas similares al Sol.\par

En el caso de las abundancias de litio, la determinación precisa de las abundancias de litio puede ser un desafío debido a la complejidad de las líneas espectrales de este elemento. Es aquí donde GES nos asiste aportando su análisis especializado y dedicado de estrellas de todos los tipos espectrales, convirtiéndose así en una herramienta fundamental a la hora de afrontar este desafío.\par

Por otra parte, tenemos información precisa sobre la velocidad angular de las estrellas. Gaia ha medido la velocidad angular de las mismas con una precisión sin precedentes, aportando así información crucial para estudiar y entender su rotación, evolución y estructura interna. Hemos combinado esta información con las abundancias de litio para así establecer hipótesis y validar las mismas acerca del impacto de la velocidad angular sobre este elemento. Paralelamente, la velocidad angular juega un papel crucial en el análisis de la presencia e intensidad de un campo magnético en las estrellas. Los campos magnéticos son responsables de la pérdida de momento angular en las estrellas.\par
...
GES DR5.0 proporciona información sobre 114 324 estrellas \citep[véase][para más detalles]{Gilmore2022}. Para nuestra investigación, estamos interesados en considerar aquéllas que pertenecen a OCs; para ello seleccionamos aquellos registros GES que cumplen alguna de las siguientes condiciones:
...
Como resultado de este primer filtrado, redujimos a 43 299 el número de estrellas potenciales. Sobre este subconjunto de datos realizamos otro filtrado, esta vez dirigido a seleccionar aquellos registros que ofrecen información no nula sobre los siguientes atributos:
...
Como datos de referencia tomamos un subconjunto de las observaciones obtenidas por GES. Este subconjunto consiste en aquellas estrellas que pertenecen a OCs, y entre ellas se encuentran potenciales gemelas ocultas de nuestro Sol. Para identificar estas últimas, procedimos a realizar una selección adicional, esta vez utilizando los valores dados por nuestros modelos durante las simulaciones para la metalicidad ($\feh$), la temperatura efectiva ($\teff$) y la gravedad superficial ($\gsurf$). Tras este último filtrado nos quedaron las estrellas para las que las observaciones de GES habían medido valores congruentes con los resultados de nuestros modelos, a partir de los cuales obtuvimos finalmente sus abundancias de litio.\par

\subsection{Selección de análogos solares} \label{sec:gemelos_solares}
Una vez preseleccionadas las estrellas pertenecientes a los OCs, nuestro interés se centró en identificar aquéllas que eran lo más parecidas a nuestro Sol cuando éste se encontraba en diferentes fases evolutivas. Nos centramos en localizar potenciales análogos solares. Esto nos llevó a cuestionarnos qué parámetros estelares debíamos tener en cuenta para clasificar una estrella como análoga solar. Encontrarlos nos ayudaría a averiguar el origen de nuestro Sol y a obtener información sobre qué condiciones prevalecían en los OCs en los que nacieron.\par

Existen trabajos previos orientados a encontrar \textit{hermanos} y \textit{gemelos} solares. Por \textit{hermanos} se refieren a estrellas que se formaron en el mismo cúmulo que el Sol y, por tanto, deberían tener edades y abundancias químicas muy similares a las de éste \citep[ver][y referencias en él]{Adibekyan2018}. Por otro lado, según \cite{Strobel1996}, se consideran \textit{gemelos} solares a aquellas estrellas que poseen parámetros físicos fundamentales como $\teff$, $\gsurf$, $\feh$, propiedades fotométricas, composición química, edad, luminosidad, rotación y campos magnéticos similares, si no idénticos, a los del Sol. Localizar estos \textit{gemelos} solares supone un reto formidable debido a los estrictos criterios y a su dispersión por toda la Vía Láctea.\par 

No es de extrañar que los métodos de búsqueda aplicados se orienten a encontrar las candidatas basándose en sus propiedades cinemáticas. Luego se comparan sus temperaturas, metalicidades y abundancias químicas con las de nuestro Sol. En nuestro estudio, ampliamos los criterios de selección añadiendo la gravedad superficial y la edad de la estrella. Estos dos nuevos parámetros aportan un ajuste fino y novedoso al proceso de búsqueda de candidatos a \textit{gemelos}. Los catálogos Gaia y GES no proporcionan la edad estimada de los OCs, así que necesitamos recurrir a los resultados obtenidos en el trabajo de \cite{Bragaglia2022} (ver figura \ref{fig:open_cluster_sample}) para obtener una referencia sobre las edades. Utilizando como referencia la edad de los OCs recogida en la tabla \ref{tab:oc_reduced_list} y los datos obtenidos a partir del conjunto de simulaciones MESA iniciadas para diferentes $\omegaini$, donde $\omegaini = \oomegac$, $\Omega$ es la velocidad angular de la estrella en la superficie estelar, y $\omegac$ es la velocidad superficial en el ecuador de una estrella en rotación donde la fuerza centrífuga equilibra la gravedad newtoniana. La edad derivada de estas simulaciones proporciona un parámetro clave para nuestro análisis. Con ella pudimos determinar los demás parámetros de simulación, que posteriormente utilizamos para compararlos con los valores asociados a las estrellas pertenecientes a los OCs preseleccionados.\par
...
De las aproximadamente 5 900 estrellas identificadas, seleccionamos aquéllas cuyos valores de $\feh$, $\teff$ y $\gsurf$ se encontraban dentro de los intervalos correspondientes definidos en la tabla \ref{tab:sel_params} y según el siguiente proceso de filtrado: a partir de los valores dados por nuestras simulaciones para los parámetros listados en la tabla \ref{tab:sel_params}, procedimos a seleccionar de los distintos OCs aquellas estrellas que tenían valores dentro de los intervalos establecidos. En concreto, para la edad estimada de un determinado cúmulo, establecimos un intervalo de $\pm$ 0.1 para OCs jóvenes (edad <\, 1 Ga) y $\pm$ 0.15 para aquéllos más viejos (edad >=\, 1 Ga) \citep[véase][para más detalle sobre los valores referidos]{Cantat-Gaudin2020}. Este intervalo dependiente de la edad sirvió como base para seleccionar los pasos de tiempo de simulación que se alineaban con los límites de edad inferior y superior de cada grupo. Una vez habíamos encontrado estos dos pasos temporales, se extrajeron los valores límite asociados para $\teff$, $\gsurf$ y $\feh$. Finalmente, con estos rangos de valores procedimos a seleccionar en cada uno de los OCs aquellas estrellas que tenían valores para estos mismos parámetros dentro del rango establecido por los valores límite. De esta forma, obtuvimos las estrellas candidatas a ser comparadas con los resultados de las simulaciones.\par

El resultado del proceso de filtrado puede observarse en las siguientes tablas. En la tabla \ref{tab:oc_reduced_list} se agrupan los OCs que aportaron alguno de sus miembros en las simulaciones que realizamos. Para un determinado OC se informa del número de estrellas que se seleccionaron en las diferentes simulaciones. Por otra parte, las tablas \ref{tab:oc_m67} y \ref{tab:oc_ngc2516} resumen el subconjunto de componentes de los OCs M67 y NGC2516 seleccionados durante las simulaciones para modelos inicializados con $\omegaini = 0.14$. Cada una de las tablas enumera el identificador GES de la estrella y los rangos de $\teff$, $\gsurf$, $\feh$, A(Li), el error estimado de la A(Li) y edad estimada utilizados en la simulación. Estos valores representan los rangos de valores en los que se ha basado el filtrado.
...
\begin{figure}
	...
	\caption{Tabla obtenida de \cite{Bragaglia2022}. Las columnas 1 a 7 listan la denominación del cúmulo, las coordenadas de ascensión recta y declinación, la edad estimada del cúmulo, la distancia (módulo de distancia convertido) en parsecs (pc) , la posición Z en coordenadas cartesianas galácticas, y la distancia desde el centro galáctico suponiendo que el Sol está a 8340 pc. Las columnas 8 y 9 enumeran el número de espectros y estrellas observadas del cúmulo, mientras que la media de [Fe/H] y la desviación estándar se dan en la columna 10.}
