...
\section{Efectos de la distribución de AML}
\subsection{Formalismo para la distribución de AML} \label{mod_aml}
Como expresa la Ec.~\ref{eq:j_dot1}, la cantidad de AML depende de $\ralfven$, $\Omega$ y $\Dot{M}$. Para valores grandes de $\ralfven$, la estrella sufre una desaceleración significativa. El valor de $\ralfven$ depende de los siguientes parámetros variables: intensidad del campo magnético ($B$), $\teff$ y $\Omega$ \citep{Gallet2015}. Con respecto a $\Dot{M}$ y como se indica en la tabla \ref{tab:phy_mesa}, para calcular la pérdida de masa se utiliza la fórmula empírica desarrollada por Reimers \citep{Reimers1975} para estrellas en su fase AGB. Para una estrella de tipo solar la $\Dot{M}$ durante la MS es relativamente pequeña, alrededor de $10^{-14}\msun \, yr^{-1}$ \citep{Noerdlinger2008}. \par
