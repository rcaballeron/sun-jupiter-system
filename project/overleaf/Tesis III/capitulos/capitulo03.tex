...
La AML causada por el MB depende directamente de la cantidad de masa perdida por la estrella debido a los vientos estelares. El coeficiente de pérdida de masa estimado para una estrella de tipo solar es de aproximadamente 10$^{-14} \msun \; yr^{-1}$ \citep{Noerdlinger2008} y la AML resultante derivado de ese valor pueden considerarse relativamente modesto como para influir decisivamente en la evolución de la estrella. Además, el efecto combinado de los movimientos convectivos y la rotación diferencial conducen a la generación de campos magnéticos. Esos campos magnéticos acaban alcanzando la superficie de la estrella, de modo que el AML puede incrementarse en varios órdenes de magnitud \citep{Langer2012}. Ese incremento es consecuencia del campo magnético que obliga a las partículas ionizadas del viento estelar a girar con la misma velocidad angular, vientos que se extienden una distancia varias veces superior al radio de la estrella \citep{UdDoula2002,Ud-Doula2007,Ud-Doula2008}.\par
...
\subsection{Formalismo de frenado magnético de intesidad fija} \label{sec:mod_mb}
Comenzamos enumerando los aspectos más relevantes y las suposiciones realizadas en la modelización de la evolución de la rotación, el frenado magnético y el momento angular (Angular Moment, AM) en MESA. Estos serán considerados para el cálculo de la AML como resultado del par aplicado por un viento estelar acoplado magnéticamente.\par 

En presencia de pérdida de masa, el material del viento estelar elimina el AM, lo que provoca un efecto de giro a la baja. Esto es muy importante, por ejemplo en las estrellas masivas, que se sabe que rotan rápidamente y tienen fuertes vientos estelares. En las siguientes secciones describimos dos enfoques semi-empíricos para el cálculo de la AML como resultado del par aplicado por un viento estelar acoplado magnéticamente.\par

\subsubsection{Frenado magnético de intesidad fija según Ud-Doula \& Owocki (2002)}
Este modelo adopta una prescripción de tipo dinamo que permite establecer una relación entre la influencia de un campo magnético dado sobre el viento estelar a través del denominado parámetro magnético de confinamiento del viento ($\eta_*$). $\eta_*$ representa una relación energética (ec. \ref{eq:wind_conf}) y define un parámetro característico de la eficacia relativa de los campos magnéticos para circunscribir y/o canalizar el flujo de salida del viento \citep{UdDoula2002}.\par
...
Por otra parte, \cite{Mathys2006} observaron en estrellas de masa intermedia con una fuerte actividad magnética una velocidad de rotación menor en comparación con otras nacidas en la misma nube parental. En esas estrellas, los campos magnéticos interactúan con la pérdida de masa que se producen en ellas, potenciando la AML. En este proceso el radio de Alfven ($\ralfven$) interpreta un rol importante: en el caso de que éste sea mayor que el radio de la estrella, entonces el flujo del viento tendrá que seguir el campo magnético. Como consecuencia, el material saldrá de la superficie estelar con un AM específico mayor, ya que el radio de co-rotación ha aumentado y corresponderá aproximadamente al ($\ralfven$), desencadenando así el efecto de MB.
...
\subsubsection{Pérdida de momento angular según Weber (1967)}

Dado que la versión de MESA utilizada en este trabajo no incluye el tratamiento de los campos magnéticos superficiales (sí tiene en cuenta el transporte por campos magnéticos internos de momento angular y elementos químicos) y por tanto, tampoco los efectos de frenado magnético, es necesario extender el simulador para incluir este fenómeno. En esta fase de la investigación hemos seguido el trabajo teórico sobre el AM en el Sol realizado por \cite{Weber1967}, haciendo uso de la ec. \ref{eq:j_dot1} para el cálculo de la AML:

\begin{equation}
 \Dot{J} = \frac{2}{3} \Dot{M}\Omega R^{2}_{A} \label{eq:j_dot1}
\end{equation}

donde $\Dot{M}$ es la tasa de pérdida de masa, $\Omega$ la velocidad angular en la superficie de la estrella y $\ralfven$ el radio Alfven. Para valores grandes de $\ralfven$, la estrella sufre una desaceleración significativa. Su valor depende de los siguientes parámetros variables: B, $\teff$ y $\Omega$ \citep{Gallet2015}. Con respecto a $\Dot{M}$, para calcular la pérdida de masa se utiliza la fórmula empírica desarrollada por Reimers \citep{Reimers1975} para estrellas en su fase AGB.

Finalmente, la ec. \ref{eq:j_dot1} se puede reescribir según \cite{Ud-Doula2008} de modo que dependa de $\eta_*$ en lugar de $R_A$, quedando así una expresión más conveniente para ser implementada en MESA porque se basa en valores directamente expuestos durante las simulaciones.
