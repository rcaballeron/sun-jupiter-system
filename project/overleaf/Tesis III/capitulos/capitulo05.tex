...
En el presente capítulo presentaremos los resultados de las simulaciones realizadas. Primeramente introduciremos la configuración aplicada a los modelos que hemos hecho evolucionar desde la PMS hasta la ZAMS. Como salida de las simulaciones analizaremos, con apoyo de diferentes gráficas, y daremos explicación detallada sobre los efectos que el frenado magnético ejerce sobre diferentes parámetros estelares, tales como temperatura efectiva ($\teff$), gravedad superficial ($\gsurf$), luminosidad ($\lstar$), velocidad angular ($\vrot$) y por supuesto, la abundancia de litio (A(Li)).\par

--¿COMPARAMOS CON OTROS TRABAJOS? SI ES ASÍ, COMENTAR AQUÍ-- \par

Finalizaremos el mismo recopilando las conclusiones más importantes derivadas de nuestra investigación y señalando posibles líneas adicionales de trabajo encaminadas a ampliar nuetra comprensión de la generación e influencia de los campos magnéticos en el devenir de estrellas similares a nuestro Sol. 

\section{Configuración de los modelos} \label{marco_teorico}
El punto de partida de nuestros modelos en los que incorporamos la simulación de un campo magnético de intensidad fija y no variable, es el que abordado previamente en la sección \ref{mesa_parte1}. Tomándolo como referencia este modelo, a su vez basado en el trabajo de \cite{Choi2016}, nos permitirá contrastar sus resultados con los nuestros. Adicionalmente, confrontaremos los mismos con las mediciones obtenidas por GES, lo que nos brindará la oportunidad de evaluar nuestro modelo tanto con trabajos teóricos previos realizados por otros investigadores, como con observaciones de campo.\par

Una vez configurado los parámetros iniciales de nuestro modelo mediante el fichero \textit{inlist} de MESA (ver resumen en la Tabla \ref{tab:phy_mesa}), éste evolucionará acorde al flujo de control del simulador. En cada paso de la simulación, comprobaremos si se dan las circunstancias apuntadas en la sección \ref{sec:mb_routine} para la activación de la rutina de frenado magnético. Si es así, calcularemos la pérdida de momento angular inducidad por el campo magnético para posteriormente distribuirla entre las capas que componen la capa convectiva de la estrella.\par
...
\subsection{Evolución alternativa del Li con MB} \label{sec:alt_mlt}
